...
def make_dataset(path, batch_size, img_size, frames, df, seed=None, years=False, model="cnn"):
    ...
    stage_discharge_train_values = [
        [np.squeeze(s)] for s in discharge_train_values]
    stage_discharge_val_values = [
        [np.squeeze(s)] for s in discharge_val_values]
    stage_discharge_test_values = [
        [np.squeeze(s)] for s in discharge_test_values]
...
def make_dataset_with_time(path, batch_size, img_size, frames, df, seed=None, years=False, model="cnn"):
    np.random.seed(seed)
    ...
    def parse_image(filename, time):
        ...
        image = tf.concat((image, new_channel), axis=2)
        image = tf.cast(image, tf.float32)
        image = image / 255
    ...
    # create stage values
    stage_train_values = [df[df.Filename == file.split(
        '/')[-1]].Stage.values for file, _ in train_files]
    stage_val_values = [df[df.Filename == file.split(
        '/')[-1]].Stage.values for file, _ in val_files]
    stage_test_values = [df[df.Filename == file.split(
        '/')[-1]].Stage.values for file, _ in test_files]

    # create discharge values
    discharge_train_values = [df[df.Filename == file.split(
        '/')[-1]].Discharge.values for file, _ in train_files]
    discharge_val_values = [df[df.Filename == file.split(
        '/')[-1]].Discharge.values for file, _ in val_files]
    discharge_test_values = [df[df.Filename == file.split(
        '/')[-1]].Discharge.values for file, _ in test_files]
...
def make_dataset_and_time(path, batch_size, img_size, frames, df, seed=None, years=False, model="cnn"):
    ...
    if years:
        ...
        # if the data is passed in order, the model just can't understand the problem so we use shuffle. Maybe because the model just can't go down the gradient because it gets confused, because the data being passed goes up then down then up then down, so it doesn't know what to do
        if model != "cnn/lstm":
            temp = list(zip(train_files, train_time))
            np.random.shuffle(temp)
            train_files, train_time = zip(*temp)
            train_files, train_time = list(train_files), list(train_time)

            temp = list(zip(val_files, val_time))
            np.random.shuffle(temp)
            ...
            test_files, test_time = list(test_files), list(test_time)
    ...
    # create time datasets (train, val, test)
    time_train_ds = tf.data.Dataset.from_tensor_slices(train_time)
    #time_train_ds = configure_for_performance(time_train_ds)
    time_val_ds = tf.data.Dataset.from_tensor_slices(val_time)
    #time_val_ds = configure_for_performance(time_val_ds)
    time_test_ds = tf.data.Dataset.from_tensor_slices(test_time)
